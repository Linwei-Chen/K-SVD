...
%                       capabilities of the machine, and performancesí
...
%NN1、NN2为图像的行数和列数
%https://ww2.mathworks.cn/help/matlab/ref/prod.html?searchHighlight=prod&s_tid=doc_srchtitle
...
%bb*bb的块转为列向量
%将随机选的8*8块转换成列放在blkMatrix里
blkMatrix = zeros(bb^2,maxNumBlocksToTrainOn);
for i = 1:maxNumBlocksToTrainOn
%ind2sub 函数确定与数组的单个索引对应的等效下标值。
%https://ww2.mathworks.cn/help/matlab/ref/ind2sub.html?searchHighlight=ind2sub&s_tid=doc_srchtitle
...
V=cos([0:1:bb-1]'*k*pi/Pn);
      ...
      %waitbar 创建进度条
      %https://ww2.mathworks.cn/help/matlab/ref/waitbar.html?searchHighlight=waitBar&s_tid=doc_srchtitle
      ...
      Weight = zeros(NN1,NN2);
      IMout = zeros(NN1,NN2);
      %[I,J] = ind2sub(siz,IND) 返回矩阵 I 和 J，其中包含与矩阵 IND（大小为 siz 的矩阵）中的每个线性索引对应的对等行下标和列下标
      [rows,cols] = ind2sub(size(Image)-bb+1,idx);
      for i  = 1:length(cols)
      col = cols(i); row = rows(i);
      block =reshape(blocks(:,count),[bb,bb]);
      IMout(row:row+bb-1,col:col+bb-1)=IMout(row:row+bb-1,col:col+bb-1)+block;
      Weight(row:row+bb-1,col:col+bb-1)=Weight(row:row+bb-1,col:col+bb-1)+ones(bb);%X = ones(n) 返回一个 n×n 的全 1 矩阵。
      count = count+1;
      end;
